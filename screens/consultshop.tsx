      ...
      {shopData && (
        <View>
          ...
          <TextInput
            ...
            onChangeText={(text) => setShopData({ ...shopData, shop_nom: text })}
            editable={isEditing}
          />

          {/* Champ Description */}
          <Text style={tw`text-lg font-bold text-gray-800 mt-4`}>𝗗𝗲𝘀𝗰𝗿𝗶𝗽𝘁𝗶𝗼𝗻</Text>
          <TextInput
            style={tw`p-2 mt-1 bg-gray-100 border border-pink-600 rounded-lg`}
            value={shopData.shop_desc}
            onChangeText={(text) => setShopData({ ...shopData, shop_desc: text })}
            editable={isEditing}
            multiline
          />

          {/* Champ Localisation */}
          <Text style={tw`text-lg font-bold text-gray-800 mt-4`}>𝗟𝗼𝗰𝗮𝗹𝗶𝘀𝗮𝘁𝗶𝗼𝗻</Text>
          <TextInput
            style={tw`p-2 mt-1 bg-gray-100 border border-pink-600 rounded-lg`}
            value={shopData.shop_local}
            onChangeText={(text) => setShopData({ ...shopData, shop_local: text })}
            editable={isEditing}
          />

          {/* Champ Numéro de téléphone */}
          <Text style={tw`text-lg font-bold text-gray-800 mt-4`}>𝗡𝘂𝗺𝗲́𝗿𝗼 𝗱𝗲 𝘁𝗲́𝗹𝗲́𝗽𝗵𝗼𝗻𝗲</Text>
          <TextInput
            style={tw`p-2 mt-1 bg-gray-100 border border-pink-600 rounded-lg`}
          ...
          <Text style={tw`text-lg font-bold text-gray-800 mt-4`}>𝗛𝗲𝘂𝗿𝗲 𝗱'𝗼𝘂𝘃𝗲𝗿𝘁𝘂𝗿𝗲</Text>
          <TextInput
            style={tw`p-2 mt-1 bg-gray-100 border border-pink-600 rounded-lg`}
            value={shopData.shop_date_ouv}
            onChangeText={(text) => setShopData({ ...shopData, shop_date_ouv: text })}
            editable={isEditing}
          />

          {/* Champ Heure de fermeture */}
          <Text style={tw`text-lg font-bold text-gray-800 mt-4`}>𝗛𝗲𝘂𝗿𝗲 𝗱𝗲 𝗳𝗲𝗿𝗺𝗲𝘁𝘂𝗿𝗲</Text>
          <TextInput
            style={tw`p-2 mt-1 bg-gray-100 border border-pink-600 rounded-lg`}
            value={shopData.shop_date_ferm}
            onChangeText={(text) => setShopData({ ...shopData, shop_date_ferm: text })}
            editable={isEditing}
          />

          {/* Bouton Modifier / Enregistrer */}
          <TouchableOpacity
            style={tw`mt-4 p-3 rounded-lg flex-row items-center ${isEditing ? 'bg-green-600' : 'bg-pink-600'}`}
            onPress={() => {
              if (isEditing) {
                handleSave(); // Sauvegarder les modifications si en mode édition
              }
              setIsEditing(!isEditing); // Activer/désactiver le mode édition
            }}
          >
            <Ionicons name={isEditing ? "checkmark-circle-outline" : "create-outline"} size={20} color="white" 
             />
            <Text style={tw`text-white ml-28`}>
